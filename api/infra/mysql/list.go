...
var ListWhere = struct {
	...
	ID:        whereHelperint64{field: "`list`.`id`"},
	UserID:    whereHelperint64{field: "`list`.`user_id`"},
	Title:     whereHelperstring{field: "`list`.`title`"},
	CreatedAt: whereHelpertime_Time{field: "`list`.`created_at`"},
	UpdatedAt: whereHelpertime_Time{field: "`list`.`updated_at`"},
}
...
var (
	listAllColumns            = []string{"id", "user_id", "title", "created_at", "updated_at"}
	listColumnsWithoutDefault = []string{"id", "user_id", "title", "created_at", "updated_at"}
...
func (q listQuery) One(ctx context.Context, exec boil.ContextExecutor) (*List, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "mysql: failed to execute a one query for list")
	}
...
func (q listQuery) All(ctx context.Context, exec boil.ContextExecutor) (ListSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "mysql: failed to assign all query results to List slice")
	}
...
func (q listQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "mysql: failed to count list rows")
	}
...
func (q listQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "mysql: failed to check if list exists")
	}
...
func (listL) LoadTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeList interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`task`),
		qm.WhereIn(`task.list_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func Lists(mods ...qm.QueryMod) listQuery {
	mods = append(mods, qm.From("`list`"))
	return listQuery{NewQuery(mods...)}
}
...
func FindList(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*List, error) {
	...
	query := fmt.Sprintf(
		"select %s from `list` where `id`=?", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "mysql: unable to select from list")
	}
...
func (o *List) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("mysql: no list provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO `list` (`%s`) %%sVALUES (%s)%%s", strings.Join(wl, "`,`"), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO `list` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `list` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, listPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "mysql: unable to insert into list")
	}
	...
	if err != nil {
		return errors.Wrap(err, "mysql: unable to populate default values for list")
	}
...
func (o *List) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("mysql: unable to update list, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE `list` SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to update list row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "mysql: failed to get rows affected by update for list")
	}
...
func (q listQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to update all for list")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to retrieve rows affected for list")
	}
...
func (o ListSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `list` SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to update all in list slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to retrieve rows affected all in update all list")
	}
	return rowsAff, nil
}
...
func (o *List) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("mysql: no list provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("mysql: unable to upsert list, could not build update column list")
		}

		ret = strmangle.SetComplement(ret, nzUniques)
		cache.query = buildUpsertQueryMySQL(dialect, "`list`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `list` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "mysql: unable to upsert for list")
	}
	...
	if err != nil {
		return errors.Wrap(err, "mysql: unable to retrieve unique values for list")
	}
...
func (o *List) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("mysql: no List provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to delete from list")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "mysql: failed to get rows affected by delete for list")
	}
...
func (q listQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("mysql: no listQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to delete all from list")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "mysql: failed to get rows affected by deleteall for list")
	}
...
func (o ListSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "mysql: unable to delete all from list slice")
	}
...
func (o *ListSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "mysql: unable to reload all in ListSlice")
	}
...
func ListExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `list` where `id`=? limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "mysql: unable to check if list exists")
	}
