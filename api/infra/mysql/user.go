...
func (o *User) Lists(mods ...qm.QueryMod) listQuery {
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`list`.*"})
	}
...
func (userL) LoadLists(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`list`),
		qm.WhereIn(`list.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on list")
	}
...
func (userL) LoadTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`task`),
		qm.WhereIn(`task.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
